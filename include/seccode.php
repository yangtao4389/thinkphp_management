...
class CCheckCodeFile
{
	...
	*/
	...
	{
		$this->mCheckCode = strtoupper(substr( str_replace(array('O','o','0','I','i','1'),'',md5(mt_rand())) ,0,$this->mCheckCodeNum));
		 
		//$this->WriteToCookie();
		session_start();
		$_SESSION["unionseccode"]=$this->mCheckCode;
		return $this->mCheckCode;
	}

	private function WriteToCookie()
	{		
		setcookie("cseccode",md5($this->mCheckCode),time()+3600,"/");
	}
	...
	*/
	private function WriteCheckCodeToImage()
	{
		for ($i=0;$i<$this->mCheckCodeNum;$i++)
		{
		  ...
		  $x = floor($this->mCheckImageWidth/$this->mCheckCodeNum)*$i + ($this->mCheckImageWidth / 20);
	...
	*/
	...
	$c_check_code_image ->SetCheckImageWH(60,20);//设置显示验证码图片的尺寸
